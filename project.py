...
def addCategory():
    #if the user isnot signed in, gets redirected to the login page.ss
	if 'username' not in login_session:
	...
	if request.method == 'POST':
		...
		if request.form['name']:
			...
			categoryToAdd = Categories(categoryName = 
							request.form['name'], categorySector = request.form['sector'],
							categoryDetails = request.form['details'],
							priceRange = request.form['pricerange'],
							categoryAgeGroup = request.form['agegroup'],
							user_id = login_session['user_id'])
			session.add(categoryToAdd)
			session.commit()
			flash("Category %s successfully added" % categoryToAdd.categoryName)
...
def editCategory(categoryName):
	categoryToEdit = session.query(Categories).filter_by(categoryName = categoryName).one()
	...
	itemsInCategory = session.query(Items).filter_by(category_name = categoryName).all()
	if request.method == 'POST':
		if request.form['name']:
            ...
            #we have to make sure that the category_name in the items table is changed as well.
			for item in itemsInCategory:
				item.category_name = request.form['name']
				session.add(item)
			session.add(categoryToEdit)
			session.commit()
			flash("Category %s successfully edited" % categoryToEdit.categoryName)
		return redirect(url_for('showAllCategories'))
...
def deleteCategory(categoryName):
	categoryToDelete = session.query(Categories).filter_by(categoryName= categoryName).one()
	...
	try:
		itemsInCategory = session.query(Items).filter_by(category_name = categoryName).one()
	...
	if request.method == 'POST':
		...
		try:
			...
			None
		session.commit()
		flash("Category %s successfully deleted" % categoryToDelete.categoryName)
		return redirect(url_for('showAllCategories'))
...
def showItems(categoryName):
	...
	items = session.query(Items).filter_by(category_name = category.categoryName).all()
...
def showDescription(categoryName,itemName):
	item = session.query(Items).filter_by(itemName = itemName, category_name = categoryName).one()
	if 'username' not in login_session:
...
def addItems(categoryName):
	...
	if request.method == 'POST':
		...
		session.commit()
		flash("Item %s successfully added in %s !" % (itemToAdd.itemName, category.categoryName))
...
def editItem(itemName, categoryName):
	itemToEdit = session.query(Items).filter_by(itemName = itemName, category_name = categoryName).one()
...
def deleteItem(categoryName, itemName):
	itemToDelete = session.query(Items).filter_by(itemName = itemName, category_name = categoryName).one()
